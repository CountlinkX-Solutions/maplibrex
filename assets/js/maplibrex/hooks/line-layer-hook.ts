/**
 * LineLayerHook - Hook para el componente LineLayer
 * 
 * Este hook gestiona una capa de líneas en MapLibre GL JS,
 * renderizando líneas con estilos configurables para rutas, caminos y conexiones.
 */
...
export const LineLayerHook: LiveViewHook = {
    ...
    try {
      ...
      const addLayer = () => {
        try {
          ...
          console.log(`[MaplibreX] Line layer "${config.id}" mounted on map "${mapId}"`);
  ...
  destroyed(this: any) {
    ...
    try {
      ...
      console.log(`[MaplibreX] Line layer "${state.config.id}" destroyed`);
    } catch (error) {
