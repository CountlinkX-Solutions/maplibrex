...
export const FillLayerHook: LiveViewHook = {
  mounted(this: any) {
    ...
    try {
      ...
      const addLayer = () => {
        try {
          ...
          // Guardar estado
          (this as any)._maplibrex_fill_layer = {
            config,
            clickHandler,
            mouseenterHandler,
            mouseleaveHandler
      ...
      console.error('[MaplibreX] Error mounting fill layer:', error);
    }
  },

  destroyed(this: any) {
    const state: FillLayerHookState | undefined = (this as any)._maplibrex_fill_layer;
    if (!state) return;
