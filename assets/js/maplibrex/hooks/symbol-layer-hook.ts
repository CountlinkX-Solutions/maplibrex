/**
 * SymbolLayerHook - Hook para el componente SymbolLayer
 * 
 * Este hook gestiona una capa de símbolos en MapLibre GL JS,
 * renderizando iconos y/o texto con estilos configurables.
 */

import type { LiveViewHook } from '../types';
import { MapManager } from '../core/map-manager';

interface SymbolLayerConfig {
  id: string;
  mapId: string;
  sourceId: string;
  sourceLayer?: string;
...
interface SymbolLayerHookState {
...
export const SymbolLayerHook: LiveViewHook = {
  mounted(this: any) {
    ...
    try {
      ...
      if (!configStr) {
        console.error('[MaplibreX] No config found on symbol layer element');
        return;
      }

      const config: SymbolLayerConfig = JSON.parse(configStr);
      const mapId = config.mapId;

      // Obtener instancia del mapa
      const map = MapManager.get(mapId);
      if (!map) {
        console.error(`[MaplibreX] Map "${mapId}" not found for symbol layer "${config.id}"`);
        return;
      }
      ...
      const addLayer = () => {
        try {
          ...
          if (!map.getSource(config.sourceId)) {
            console.warn(`[MaplibreX] Source "${config.sourceId}" not found for symbol layer "${config.id}". Layer will be added when source is available.`);
            return;
          }
          ...
          // Guardar estado
          (this as any)._maplibrex_symbol_layer = {
            config,
            clickHandler,
            mouseenterHandler,
            mouseleaveHandler
          ...
          console.log(`[MaplibreX] Symbol layer "${config.id}" mounted on map "${mapId}"`);

        } catch (error) {
          console.error(`[MaplibreX] Error adding symbol layer "${config.id}":`, error);
        }
      };
      ...
      console.error('[MaplibreX] Error mounting symbol layer:', error);
    }
  },

  destroyed(this: any) {
    const state: SymbolLayerHookState | undefined = (this as any)._maplibrex_symbol_layer;
    if (!state) return;
    ...
    try {
      ...
      console.log(`[MaplibreX] Symbol layer "${state.config.id}" destroyed`);
    } catch (error) {
      console.error('[MaplibreX] Error destroying symbol layer:', error);
    }
  }
};
