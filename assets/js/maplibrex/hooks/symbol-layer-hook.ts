/**
 * SymbolLayerHook - Hook for the SymbolLayer component
 * 
 * This hook manages a symbol layer in MapLibre GL JS,
 * rendering icons and/or text with configurable styling.
 */
...
export const SymbolLayerHook: LiveViewHook = {
    ...
    try {
      ...
      const addLayer = () => {
        try {
          ...
          logger.debug(`[MaplibreX] Symbol layer "${config.id}" mounted on map "${mapId}"`);
  ...
  destroyed(this: any) {
    ...
    try {
      ...
      logger.debug(`[MaplibreX] Symbol layer "${state.config.id}" destroyed`);
    } catch (error) {
